...
export default function DashboardView({ sensors, onSelectSensor, onRefresh }) {
  ...
  const stats = useMemo(() => {
    ...
    sensors.forEach((s) => {
      const st = evaluateStatus(s.currentValue);
      if (st === 'Healthy') healthy++;
      else if (st === 'Warning') warning++;
      else if (st === 'Critical') critical++;
    });

    return { total: sensors.length, healthy, warning, critical };
  }, [sensors]);

  // Filter sensors list
  const filteredSensors = useMemo(() => {
    return sensors.filter((s) => {
      const matchesSearch = s.device_name.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      <div style={{
        ...
        <div>
          <h1 style={{
            ...
            <span>Gas Telemetry Dashboard</span>
            <span style={{
              fontSize: '11px',
              padding: '3px 8px',
              borderRadius: '6px',
          ...
          <p style={{ fontSize: '13px', color: 'var(--text-muted)', margin: 0 }}>
            Continuous H2S Gas monitoring for industrial systems
          </p>
        </div>
      ...
      <div style={{
        ...
        >
          <div>
            <span style={{ fontSize: '12px', color: 'var(--text-muted)', fontWeight: 600 }}>Warning (10 - 20 PPM)</span>
        ...
        >
          <div>
            <span style={{ fontSize: '12px', color: 'var(--text-muted)', fontWeight: 600 }}>Critical (≥ 20 PPM)</span>
      ...
      {filteredSensors.length > 0 ? (
        ...
        <div style={{
          ...
          <h3 style={{ fontSize: '16px', color: 'var(--text-main)', margin: '0 0 6px 0' }}>No Sensors Found</h3>
